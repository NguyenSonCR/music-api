...
router.get("/", async (req, res) => {
    try {
        ...
        if (data.msg) {
            return res.json({
                success: true,
                message: "Lấy dữ liệu cho trang chủ thành công",
                homeData: data.data,
            });
        } else {
            return res
...
router.get("/top100", async (req, res) => {
    try {
        ...
        if (data.msg) {
            return res.json({
                success: true,
                message: "Lấy dữ liệu top 100 thành công",
                data: data.data,
            });
        } else {
            return res
...
router.get("/newrelease", async (req, res) => {
    try {
        ...
        Promise.all([dataSongPromise, dataAlumPromise])
            .then((array) => {
                return res.json({
                    success: true,
                    message: "Lấy dữ liệu bài hát mới phát hành thành công",
                    data: {
                        song: array[0].data,
                        album: array[1].data,
                    },
...
router.get("/album/:id", async (req, res) => {
    ...
    try {
        ...
        if (response.msg) {
            return res.json({
                success: true,
                message: "Lấy dữ liệu playlist thành công",
                data: response.data,
            });
        } else {
            return res
...
router.get("/song/:id", async (req, res) => {
    ...
    try {
        ...
        if (response.msg === "Success" && info.msg) {
            return res.json({
                success: true,
                message: "Lấy dữ liệu bài hát thành công",
                data: response.data,
                info: info.data,
            });
        } else {
            return res.status(200).json({
                success: false,
                message: "Bài hát chỉ dành cho tài khoản VIP",
            });
        }
    } catch (error) {
        console.log(error);
...
router.get("/song/lyric/:id", async (req, res) => {
    ...
    try {
        ...
        if (response.msg) {
            return res.json({
                success: true,
                message: "Lấy dữ liệu lời bài hát thành công",
                lyric: response.data,
            });
        } else {
            return res.status(200).json({
...
router.get("/hub", async (req, res) => {
    try {
        const response = await ZingMp3.getHub();
        if (response.msg) {
            return res.json({
                success: true,
                message: "Lấy dữ liệu thể loại nhạc thành công",
...
router.get("/genres/:id", async (req, res) => {
    ...
    try {
        ...
        if (response.msg) {
            return res.json({
                success: true,
                message: "Lấy dữ liệu chi tiết thể loại nhạc thành công",
                genresDetail: response.data,
            });
        } else {
            return res
                .status(200)
    ...
    try {
        ...
        if (response.msg) {
            return res.status(200).json({
                success: true,
                data: response.data,
                message: "Lấy dữ liệu ca sĩ thành công",
            });
        }
    } catch (error) {
        console.log(error);
...
router.patch("/mymusic/library/add", verifyToken, async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({
            success: true,
            message: "Đã thêm mới bài hát vào thư viện thành công",
            user: userUpdated,
        });
    } catch (error) {
        console.log(error);
...
router.patch("/mymusic/library/remove", verifyToken, async (req, res) => {
    ...
    try {
        ...
        const updated = {
            music: {
                playlist: user.music.playlist,
                library: user.music.library.filter(
        ...
        return res.status(200).json({
            success: true,
            message: "Đã xóa bài hát khỏi thư viện thành công",
            user: userUpdated,
        });
    } catch (error) {
        console.log(error);
